...
files.read_file(st)# Lee el archivo para cargar las actividades ya existentes
...
def main_window():
    ...
    tk.Button(main, text="Graficos", command=lambda:[win_graphs(list_activities(st))], height=2,width=20).place(x=168, y=240)
...
def win_graphs(list):
    winmenuactivities = tk.Toplevel()
    winmenuactivities.title("Graficos actividades")
    winmenuactivities.minsize(700,400)
    fontStyle = tkFont.Font(family="Lucida Grande", size=12)
    listac = list_activities(st)
    lb_activities= tk.Label(winmenuactivities,text="Actividad:").place(x=10, y=40)
    ...
    tk.Button(winmenuactivities, text="Generar grafico",font=fontStyle, command=lambda:[graphs.gen_graphs(select_activities(sv_activities,st))],height=2,width=14).place(x=85, y=130)
    tk.Button(winmenuactivities, text="Salir",font=fontStyle, command=lambda:close(winmenuactivities),height=2,width=14).place(x=85, y=210)
def select_activities(ac,student):
        ac = ac.get()
        ac = ac.split("/")
